function [empirical_values_session_subj]=parfor_BMR_paired_permutation_session_paper_variability(subject,N_permutations,GCM_group_basic,A_matrix_group,A_matrix_group_BMR)
    
empirical_values_session_subj=zeros(0,N_permutations);
for permutation=1:N_permutations
    ...
    perm_matrix_session_1(:,find(perm==0))=A_matrix_group_BMR{subject}(:,find(perm==0));
    
    perm_matrix_session_2(:,find(perm==0))=A_matrix_group{subject}(:,find(perm==0));
    perm_matrix_session_2(:,find(perm==1))=A_matrix_group_BMR{subject}(:,find(perm==1));
