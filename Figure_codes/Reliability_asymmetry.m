...
box1=boxplot(A,y_axis,x_axis,'PlotStyle','traditional','Colors','b','OutlierSize',0.00001,'whisker',10);

h = findobj(A,'Tag','Box');
for j=1:length(h)
    p(j)=patch(get(h(j),'XData'),get(h(j),'YData'),'b','FaceAlpha',.3);
    p(j).Parent=A;
end

%Change colors median-lines
...
%Change color whisker
lower_whisker = findobj(gcf, 'type', 'line', 'Tag', 'Lower Whisker');
lower_whisker_end = findobj(gcf, 'type', 'line', 'Tag', 'Lower Adjacent Value');

upper_whisker = findobj(gcf, 'type', 'line', 'Tag', 'Upper Whisker');
upper_whisker_end = findobj(gcf, 'type', 'line', 'Tag', 'Upper Adjacent Value');

set(lower_whisker, 'Color', 'b');
set(lower_whisker, 'LineWidth',3);

set(lower_whisker_end, 'Color', 'b');
set(lower_whisker_end, 'LineWidth',3);


set(upper_whisker, 'Color', 'b');
set(upper_whisker, 'LineWidth',3);

set(upper_whisker_end, 'Color', 'b');
set(upper_whisker_end, 'LineWidth',3);

%Change color boxes
boxes=findobj(gcf, 'type', 'line','Tag','Box');
set(boxes,'LineWidth',3);
set(boxes,'LineWidth',3,'color','b');

Ylim=A.YLim;
set(A,'YLim',[Ylim(1), Ylim(2)+0.4])
