...
load([Work_dir '/results_perm_tests/Asymmetry/session/adj_p_adapt.mat']);

figure('units','normalized','outerposition',[0 0 1 1]);

a=plot(([cr_group(:,find((adj_p_adapt<0.05)));cr_group_delat(:,find((adj_p_adapt<=0.05)))]),'-x','LineWidth',4,'MarkerSize',30);
a=plot(([cr_group(:,find((adj_p_adapt<0.05)));cr_group_delat(:,find((adj_p_adapt>0.95)))]),'--x','LineWidth',4,'MarkerSize',30);
hold on;
