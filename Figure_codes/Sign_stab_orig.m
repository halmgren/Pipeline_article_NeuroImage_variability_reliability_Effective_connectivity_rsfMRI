function Sign_stab_orig(SPM_dir,Work_dir)
...
for sz1=1:4
    for sz2=1:4
        number_nega(sz1,sz2)=sum(squeeze(sign(A_matrix(sz1,sz2,:)))==-1);
        number_posi(sz1,sz2)=sum(squeeze(sign(A_matrix(sz1,sz2,:)))==1);
        number_NaN(sz1,sz2)=sum(isnan(squeeze(A_matrix(sz1,sz2,:))));
...
prop_posi=number_posi/length(Lat_group);
prop_nega=number_nega/length(Lat_group);
prop_NaN=number_NaN/length(Lat_group);
...
for sz1=1:4
    for sz2=1:4
        if prop_posi(sz1,sz2)>=0.7
            sign_stab(sz1,sz2)=0.1;
%         elseif prop_posi(sz1,sz2)>0.6
%             sign_stab(sz1,sz2)=2;
%         elseif prop_nega(sz1,sz2)>0.8
%             sign_stab(sz1,sz2)=3;
        elseif prop_nega(sz1,sz2)>=0.7
            sign_stab(sz1,sz2)=2;
        end
    end
end
...
for i=0:5
    line([0.5+i 0.5+i],[0.5 4.5],'Color','k','LineWidth',5);
    line([0.5 4.5],[0.5+i 0.5+i],'Color','k','LineWidth',5);
end
...
if size(A_matrix,1) == size(A_matrix,2) && ...
    ...
    set(gca,'YTickLabel',regions,...
        ...
        'XTickLabel',regions,'fontweight','bold','fontsize',34,'XTick',...
...
saveas(gcf,[Work_dir '/Figures_paper_variability/Sign_stability_dominance_orient.bmp']);
close;
end
