...
for subject=1:17
    ...
    EP=full(vec2mat(PEB_group{subject}.Ep(1:16),4)');
    CP=diag(PEB_group{subject}.Cp);
    CP=full(vec2mat(CP(1:16),4)');
    sgn=sign(EP-ci*sqrt(CP)).*sign(EP+ci*sqrt(CP));
    
    A_matrix=full(vec2mat(PEB_group{subject}.Ep(1:16),4)');
    ...
    saveas(gcf,[Work_dir '/Figures_paper_variability/Supplementary_Figures/Subject_' sprintf('%02i',subject) '_PEB_Ep.bmp']);
    close;
    clearvars -except subject GCM PEB_group Work_dir SPM_dir;
end
end